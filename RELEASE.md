...
```bash
npx ovsx publish xinghan-upload-0.14.0.vsix --no-dependencies
```
...
# 或命令行传入
npx ovsx publish xinghan-upload-0.14.0.vsix --no-dependencies -p 你的令牌
```
...
- [ ] `package.json` 中 `version` 已改为新版本（如 `0.14.0`）
