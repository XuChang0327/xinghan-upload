...
- **星瀚助手**（上栏）
  ...
  - **🔌 连接 REPL**：选择星瀚控制器端口后在内置终端进入 REPL
  - **📴 断开 REPL**：断开 REPL 终端并释放串口，便于随后上传或运行
...
- **连接状态**（下栏）
  ...
  - 标题栏 **刷新** 按钮：仅刷新端口列表展示，与是否连接 REPL 无关
  - 连接 REPL 后会显示「已连接端口」
...
### v0.8.0
- **连接状态**栏：新增独立面板，初始即展示可用端口（星瀚控制器）；标题栏「刷新」按钮仅刷新端口列表，与连接 REPL 无关；连接 REPL 后显示「已连接端口」
- **连接 REPL**：原「串口通信」更名为「连接 REPL」
- **断开 REPL**：新增入口与断开图标，可释放串口以便上传或运行
