...
export class XinghanActionsTreeProvider implements vscode.TreeDataProvider<ActionItemNode> {
  ...
  getChildren(): ActionItemNode[] {
    ...
    const replItem: ActionItemNode = this._isReplConnected
      ? { id: "toggleRepl", label: "REPL断开", command: "xinghan.toggleRepl", tooltip: "关闭 REPL 终端", icon: "debug-disconnect" }
      : { id: "toggleRepl", label: "REPL连接", command: "xinghan.toggleRepl", tooltip: "进入 MicroPython REPL（有线/蓝牙）", icon: "terminal" };
    return [
      runItem,
      ACTION_ITEMS[0],
      ACTION_ITEMS[1],
