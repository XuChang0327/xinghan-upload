...
function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand("xinghan.upload", async () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand("xinghan.runOnDevice", async () => {
        if (runOnDeviceProcess) {
            channel.show(true);
    ...
    // 列出可用串口（供排查或配置用）
    context.subscriptions.push(vscode.commands.registerCommand("xinghan.listPorts", async () => {
        const config = getConfig();
        const scriptPath = resolveScriptPath(context.extensionPath);
        channel.show(true);
        channel.clear();
        channel.appendLine("正在列出串口...");
        const { exitCode, stdout } = await runPythonScript(config.pythonPath, scriptPath, ["--list-ports"], channel);
        if (exitCode === 0) {
            try {
                ...
                const chosen = await vscode.window.showQuickPick(ports.map((p) => ({ label: p.device, description: p.description, device: p.device })), { title: "选择串口（可复制到设置 xinghan.serialPort）", matchOnDescription: true });
                if (chosen) {
